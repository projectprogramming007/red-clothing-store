    ...
    public class UsersProvider
    {
        ...
        public static bool NewUser(string userID, string password)
        {
            try
            {
                ...
                string hashedPassword = EncriptionProvider.ComputeHash(password, EncriptionProvider.Supported_HA.SHA256, null);

                // Create a new UserEntity object
                UserEntity newUser = new UserEntity(userID, hashedPassword);

                // Add the new user to the database
                bool userAdded = DatabaseConnection.Add<UserEntity>(newUser);

                return userAdded;
            }
            catch (Exception ex)
            {
                // Log any exceptions that occur during the user addition process
                log.Error("An error occurred while adding a new user: " + ex.Message);
